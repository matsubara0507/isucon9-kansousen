...
func getUserSimplesByItems(q sqlx.Queryer, items *[]Item, userSimple UserSimple) (map[int64]UserSimple, error) {
	...
	userIDMap[0] = false
	userIDMap[userSimple.ID] = false

	userIDs := make([]int64, len(userIDMap))
	for idx, ok := range userIDMap {
		if ok {
...
func postItemEdit(w http.ResponseWriter, r *http.Request) {
	...
	targetItem := Item{}
	err = dbx.Get(&targetItem, "SELECT `id`, `seller_id`, `status` FROM `items` WHERE `id` = ?", itemID)
	if err == sql.ErrNoRows {
		outputErrorMsg(w, http.StatusNotFound, "item not found")
		return
	}
...
func postShip(w http.ResponseWriter, r *http.Request) {
	...
	item := Item{}
	err = dbx.Get(&item, "SELECT `id`, `seller_id`, `status` FROM `items` WHERE `id` = ?", itemID)
	if err == sql.ErrNoRows {
		outputErrorMsg(w, http.StatusNotFound, "item not found")
		return
	}
...
func postComplete(w http.ResponseWriter, r *http.Request) {
	...
	item := Item{}
	err = dbx.Get(&item, "SELECT `id`, `buyer_id`, `status` FROM `items` WHERE `id` = ?", itemID)
	if err == sql.ErrNoRows {
		outputErrorMsg(w, http.StatusNotFound, "items not found")
		return
	}
...
func postBump(w http.ResponseWriter, r *http.Request) {
	...
	targetItem := Item{}
	err = dbx.Get(&targetItem, "SELECT `id`, `seller_id`, `price` FROM `items` WHERE `id` = ?", itemID)
	if err == sql.ErrNoRows {
		outputErrorMsg(w, http.StatusNotFound, "item not found")
		return
	}
	...
	seller := User{}
	err = dbx.Get(&seller, "SELECT `id`, `last_bump` FROM `users` WHERE `id` = ?", userID)
	if err == sql.ErrNoRows {
		outputErrorMsg(w, http.StatusNotFound, "user not found")
		return
	}
...
func postLogin(w http.ResponseWriter, r *http.Request) {
	...
	u := User{}
	err = dbx.Get(&u, "SELECT `id`, `hashed_password` FROM `users` WHERE `account_name` = ?", accountName)
	if err == sql.ErrNoRows {
